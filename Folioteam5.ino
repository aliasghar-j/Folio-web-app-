...
#include <PDM.h>
#include <podapaithiyo-project-1_inferencing.h>
#include <SPI.h>
#include <Wire.h>
...
void setup()
{
    ...
    Serial.println("March 2024 @Folio");
    Serial.println("-----------------------");
  ...
  display.println("Folio");
  display.println("Machine Learning"); 
  display.println("by FOLIO"); 
  display.display();
  delay(1000); 
...
void loop()
{
    ...
    for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
         ei_printf("    %s: %.5f\n", result.classification[ix].label, result.classification[ix].value);
          ...
          if (strcmp(result.classification[ix].label,"hojas")==0){
             hojas=result.classification[ix].value;
             ei_printf("Detected 'hojas' with confidence %.5f\n", hojas);
          }
          
          if (strcmp(result.classification[ix].label, "silencio") == 0) {
            silencio = result.classification[ix].value;
            ei_printf("Detected 'silencio' with confidence %.5f\n", silencio);
          }
    ...
    // Calculate remaining minutes based on elapsed time and pages read
    int remainingMinutes = (ElapsedTime / pagesCounter) * (pages - pagesCounter) / 1000 / 60;

    // Clear the display and show the remaining time
    display.clearDisplay();
    display.setCursor(0, 0);
...
} else if (silencio > 0.30) {  // If silence is detected
    display.println("Reading Silence");
    display.display();
} else {
    display.println("No clear result");
    display.display();
}
